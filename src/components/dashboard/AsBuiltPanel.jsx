...
export default function AsBuiltPanel() {
  ...
  return (
    <div className="bg-panel rounded-lg p-3">
      <div className="text-[10px] uppercase tracking-wider text-slate-400">As-Built Plant</div>
      ...
      <div className="grid gap-1.5">
        {specs.map((spec) => {
          ...
          const dev = (m - 1) * 100;
          const devColor = Math.abs(dev) < 0.05 ? 'text-slate-500' : dev < 0 ? 'text-warn' : 'text-safe';
          return (
            <div key={spec.key} className="text-[10px]">
              <div className="flex justify-between items-baseline">
                <span className="text-slate-300">
                  {spec.label}
                  <span className="text-[8px] text-slate-500 italic ml-1">({spec.tech})</span>
                </span>
              ...
              <div className="flex justify-between text-[9px] font-mono">
                <span className="text-slate-500">design {design}</span>
                <span className="text-slate-200">as-built {actual}</span>
              </div>
              <p className="text-[8px] text-slate-600 leading-snug">{spec.note}.</p>
            </div>
          );
        })}
      </div>
