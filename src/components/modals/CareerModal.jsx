    ...
    <div className={`flex items-start gap-2 ${dim ? 'opacity-45' : ''}`}>
      <div
        className={`w-6 h-6 shrink-0 rounded-full flex items-center justify-center text-[10px] font-bold border ${
          ...
          : state === 'current' ? 'bg-accent/20 border-accent text-accent'
      ...
      <div className="min-w-0">
        <div className="text-[11px] font-semibold leading-tight flex items-center gap-1.5 flex-wrap">
          {posting.title}
          {state === 'current' && (
            <span className="text-[8px] font-bold tracking-widest text-accent bg-accent/15 rounded px-1 py-px">CURRENT</span>
          )}
          {state === 'next' && (
            <span className="text-[8px] font-bold tracking-widest text-ink/70 bg-raise rounded px-1 py-px">NEXT</span>
          )}
          {state === 'past' && (
            <span className="text-[8px] font-bold tracking-widest text-ink/55 bg-panel rounded px-1 py-px">JOINED ABOVE THIS RUNG</span>
          )}
          {state === 'future' && (
            <span className="text-[8px] font-bold tracking-widest text-ink/55 bg-panel rounded px-1 py-px">IN DEVELOPMENT</span>
          )}
          {grade && (
        ...
        <div className="text-[9px] text-ink/55 leading-snug">{posting.org}. {posting.tagline}</div>
      </div>
    </div>
  );
}
...
function RecommendedCases({ posting }) {
  ...
  return (
    <div className="mt-3">
      <div className="text-[9px] uppercase tracking-widest text-ink/55 mb-1">
          ...
          return (
            <button
              ...
              className={`text-[10px] px-2 py-1 rounded border ${
                read ? 'border-safe/40 text-safe bg-safe/5' : 'border-raise-hi text-ink/85 hover:bg-raise'
              }`}
      ...
      {posting.mandateNote && (
        <p className="text-[9px] text-ink/55 mt-1.5 leading-snug italic">{posting.mandateNote}</p>
      )}
    </div>
  );
}
...
function ResponsibilityBlock({ posting }) {
  ...
  if (!r) return null;
  return (
    <div className="mt-2.5 pt-2 border-t border-raise">
      <div className="text-[9px] uppercase tracking-widest text-ink/55 mb-1">
        How your responsibilities change
      </div>
      {posting.roleLine && (
        <p className="label-mono text-[9px] text-accent mb-1.5">{posting.roleLine}</p>
      )}
      <div className="grid gap-0.5">
        ...
        {r.shed.map((g) => (
          <p key={g} className="text-[10px] text-ink/55 leading-snug">− {g}</p>
        ))}
      </div>
    </div>
  );
...
function ReviewCard({ posting, review }) {
  return (
    <div className="border border-raise-hi bg-base/70 p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[9px] uppercase tracking-[0.25em] text-ink/55">Performance review</div>
        ...
        <div className={`w-12 h-12 shrink-0 border-2 flex flex-col items-center justify-center ${GRADE_COLOR[review.grade]}`}>
      ...
      <p className="text-[11px] text-ink/85 italic mt-2 leading-relaxed">
        “{review.summary}”
        {posting.boss && (
          <span className="not-italic text-ink/55"> · {posting.boss.name}, {posting.boss.role}</span>
        )}
      </p>

      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 mt-3 bg-base/60 border border-raise p-2">
      ...
      <div className="grid sm:grid-cols-2 gap-3 mt-3">
        <div>
          ...
          {review.strengths.map((t) => (
            <p key={t} className="text-[10px] text-ink/85 leading-snug mb-1">+ {t}</p>
          ))}
        </div>
        <div>
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/85 flex items-center justify-center p-4">
      <motion.div
        ...
        className="bg-panel border border-accent/40 max-w-xl w-full p-5 max-h-[92vh] overflow-y-auto"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-[9px] text-ink/55">One engineer, many machines. The notebook follows you.</div>
          </div>
          <button
            onClick={() => setCareerOpen(false)}
            aria-label="Close the career file"
            className="p-1.5 rounded bg-raise hover:bg-raise-hi"
          >
        ...
        {offer ? (
          <div className="mt-4 grid gap-3">
            ...
            {next ? (
              <div className="border border-accent/40 bg-accent/5 p-4">
                <div className="text-[9px] uppercase tracking-[0.25em] text-accent">Promotion offer</div>
                <div className="text-sm font-bold mt-0.5">{next.title}</div>
                <div className="text-[10px] text-ink/70">{next.org}</div>
                <div className="mt-2 grid gap-1">
                  {next.mandate.map((m) => (
                    <p key={m} className="text-[10px] text-ink/85 leading-snug">▸ {m}</p>
                  ))}
                </div>
                ...
                <button
                  ...
                  autoFocus
                  className="mt-3 w-full py-2.5 bg-accent text-base font-bold text-sm tracking-widest hover:brightness-110"
                >
                  ACCEPT PROMOTION
                </button>
                <p className="text-[9px] text-ink/55 mt-1.5 text-center">
              ...
              <div className="border border-raise-hi bg-base/70 p-4">
                ...
                <p className="text-[11px] text-ink/85 leading-relaxed mt-1.5">{CAREER_EPILOGUE}</p>
                <button
                  onClick={() => setCareerOpen(false)}
                  autoFocus
                  className="mt-3 w-full py-2 bg-accent text-base font-bold text-xs tracking-widest hover:brightness-110"
                >
                  CONTINUE OPERATING
                </button>
              </div>
          ...
          <div className="mt-4 border border-raise-hi bg-base/70 p-4">
            <div className="text-[9px] uppercase tracking-[0.25em] text-ink/55">Posting brief</div>
            <div className="text-sm font-bold mt-0.5 flex items-center gap-2">
              {posting.title}
              <SpeakerIcon text={`Posting brief. ${posting.title}. ${posting.org}. ${posting.mandate?.join(' ') ?? ''}`} />
            </div>
            <div className="text-[10px] text-ink/70">{posting.org} · Mission {levelId} of {missionCount}</div>
            <div className="mt-2 grid gap-1">
        ...
        <div className="mt-4">
          <div className="text-[9px] uppercase tracking-widest text-ink/55 mb-2">The ladder</div>
        ...
        {career.completed.some((c) => c.postingId !== career.postingId) && (
          <div className="mt-4">
            <div className="text-[9px] uppercase tracking-widest text-ink/55 mb-1">Reviews on file</div>
            {career.completed.filter((c) => c.postingId !== career.postingId).map((c) => (
              <p key={c.postingId} className="text-[10px] text-ink/70 leading-snug mb-0.5">
        ...
        <div className="mt-4 pt-3 border-t border-raise flex items-center justify-between">
          <button
            onClick={exportNotebook}
            className="text-[10px] px-2.5 py-1 rounded-full font-semibold bg-raise text-ink hover:bg-raise-hi"
            title="Download the full career log as a text file"
          >
            Export career log
          </button>
          <span className="text-[9px] text-ink/40">Grades go in the file, never in your way.</span>
        </div>
      </motion.div>
    </div>
  );
